...
<div class="am-cf admin-main">
    ...
    <div class="admin-content">
        ...
        <div class="am-g">
            ...
            <!-- <div class="am-u-sm-12 am-u-md-3">
                    <div class="am-input-group am-input-group-sm">
                        ...
                        <form id="search" action="admin/admin_order_search" method="post">
                            <input type="text" name="order_search_key" class="am-form-field">
                            <input type="submit" value="搜索">
                            <span class="am-input-group-btn">
                            <button class="am-btn am-btn-default" type="button"><input type="submit" value="搜索"></button>
                            </span> 
                        </form>
                    </div>
            </div> -->
        ...
        <div class="am-g" style="height: 1500px;">
                ...
                <table class="am-table am-table-striped am-table-hover table-main">
                    <thead>
                        <tr>
                            ...
                            <th class="table-type">操作<button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" id="doc-prompt-toggle" data-am-popover="{content: '请您在删除前输入管理员登录密码！', trigger: 'hover focus'}"><span class="am-icon-key"></span> 验证</button>
            ...
            <span id="result">共&lt;<?php echo $order_count;?>&gt;条结果</span>
    ...
    $(function(){
        $('.am-btn-delete').on('click', function(){
            ...
            if(confirm('确定是否删除记录，不可恢复!?')){
                location.href = 'admin/delete_order/'+orderId;
            }
        });
...
/*****批量删除*****/
        ...
        $delCheckedBtn.on('click', function(){
            ...
            $.get('admin/remove_checked_orders?Ids='+$orderIds,function(res){
                if(res == 'success'){
...
//     $('#order-status li').on('click',function(){
    //         var $orderStatus = $(this).data('color');

    //         //alert();
    //         //$('.order-status').addClass'('+$orderStatus+')';

    //         $('.order-status').attr("class",$orderStatus );
    //     });
...
<script>
    ...
    //     // alert($order_color+','+$order_id);
    //     //$('#texture-detail').find("input:hidden").val($items_id);
    //     location.href = 'admin/update_order_status?order_id='+$order_id+'&order_color='+$order_color;    
    // });
</script>
...
<script>
    ...
    $('.order-status').on('change',function(){
        ...
        $.get('admin/update_order_status1',{'order_color':$order_color,'order_id':$order_id},function(res){
            if(res){
                //console.log(res.data[0].order_color);
